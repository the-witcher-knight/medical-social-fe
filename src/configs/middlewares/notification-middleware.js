...
export default () => next => action => {
  ...
  if (isFulfilledAction(action) && payload && payload.headers) {
    ...
    let alert = null; // string | null
    let alertParams = null; // string | null
    headers &&
    ...
    if (alert) {
      ...
      toast.success(alert + ', ' + alertParam);
    }
  }
      ...
      if (
        !(
          ...
          (error.message === '' ||
            (data &&
              data.path &&
              (data.path.includes('/api/account') || data.path.includes('/api/authenticate'))))
        )
      ) {
